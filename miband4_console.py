...
sock.bind(('', 8008))  # I'm using port 12345 to bind to

# Define a function for the thread
def listening_thread():
         # data needs to be defined as global inside the thread
    global data
    while True:
...
def display_vib_once():
    #band.send_custom_alert()
    while True:
        data_raw, addr = sock.recvfrom(1024)
        datas = data_raw.decode()    # My test message is encoded
        print ("Received message inside thread:", datas)
    
        band.send_alert('\x08\x03')
        time.sleep(1)
   
#    while(True):
 #       if int(data) != offset:
  #          data = int(offset)
   #         band.send_alert(b'\x80\x01')
    #        time.sleep(offset)
     #       band.send_alert(b'\0x13')
...
if __name__ == "__main__":
    ...
    vib_once = FunctionItem("@ display vibration once", display_vib_once)
    vib_offset = FunctionItem("@ display vibration at offset from udp", vib_at_offset)
    ...
    menu.append_item(vib_once)
    menu.append_item(vib_offset)
